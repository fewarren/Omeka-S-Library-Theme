...
(function() {
    ...
    console.log('Enhanced Preset System: Initializing...');
    
    // Preset definitions matching layout.phtml
    const PRESETS = {
        traditional: {
            // Typography
    ...
    function setFieldValue(name, value) {
        ...
        if (typeof name !== 'string' || name.trim() === '') {
            console.error('Enhanced Preset: Invalid field name provided');
            return false;
        }
        ...
        if (!field) {
            console.warn(`Enhanced Preset: Field not found: ${name}`);
            return false;
        }
        ...
        if (field.type === 'checkbox') {
            ...
            if (typeof value === 'string' && /^#[0-9A-Fa-f]{6}$/.test(value)) {
                ...
                console.warn(`Enhanced Preset: Invalid color format for ${name}: ${value}`);
                return false;
            }
        } else {
        ...
        console.log(`Enhanced Preset: Set ${name} = ${sanitizedValue}`);
        return true;
    }
    ...
    function applyPreset(presetName) {
        ...
        if (!preset) {
            console.error(`Enhanced Preset: Unknown preset: ${presetName}`);
            return;
        }
        
        console.log(`Enhanced Preset: Applying ${presetName} preset...`);
        ...
        console.log(`Enhanced Preset: Applied ${appliedCount}/${totalCount} settings`);
            ...
            applyPresetField.addEventListener('change', function() {
                if (this.checked && !isApplying) {
                    ...
                    try {
                        ...
                        applyPresetAsync()
                            .then((result) => {
                                console.log(`Enhanced Preset: Successfully applied ${selectedPreset} preset`);
                            })
                            .catch((error) => {
                                console.error(`Enhanced Preset: Error applying ${selectedPreset} preset:`, error);
                        ...
                        console.error('Enhanced Preset: Immediate error:', error);
                        this.checked = false;
                        this.disabled = originalDisabled;
                        isApplying = false;
